<template>
  ...
  <GlobalContainer class="all-campaigns">
    ...
    <CampaignsFilters v-if="status == 'success'" :orphanType="'family'" :filter="filter" :submit="submit" />
...
import { useFamilyCampaigns } from "../services/family_campaigns";
...
const { campaigns, campaignsMeta, refresh, status } = useFamilyCampaigns();
...
const filter = reactive({
  ...
  age: Array.isArray(route.query.age) ? route.query.age[0] : route.query.age || "",
});
...
function getFilter() {
  const object = {
    ...filter,
    min_age:
      (Array.isArray(filter.age) ? filter.age[0] : filter.age)?.split("-").at(0) || "",
    max_age:
      (Array.isArray(filter.age) ? filter.age[0] : filter.age)?.split("-").at(-1) || "",
...
// SEO
siteName("global.family_campaigns");
